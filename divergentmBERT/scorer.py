...
class DivergentmBERTScorer:
    ...
    def compute_divergentscore(self, texts_a, texts_b, explain_source):

        processor = processors[self.task_name.lower()]()
        examples = []

        for (i, (text_a, text_b)) in enumerate(zip(texts_a, texts_b)):
            if explain_source: a, b = text_a[0], text_b
            else: a, b = text_a, text_b[0]

            guid = "%s-%s" % ('test', i)
            examples.append(
                InputExample(guid=guid, text_a=a, text_b=b, label='0'))
        #print(examples)
